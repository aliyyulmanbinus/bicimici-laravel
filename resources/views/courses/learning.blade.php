...
@section('title', 'Course Learning - bicimici LMS')
@section('content')
<div class="flex h-screen">
    <aside class="flex flex-col border border-bicimici-grey bg-white">
        <div class="w-[260px] pb-[20px] h-[280px] px-5 pt-5 flex flex-col gap-5">
            <ul>
                <li>
                    <a href="{{ route('dashboard') }}">
                        <div class="flex items-center gap-2 py-[10px] px-[14px] rounded-full border border-bicimici-grey bg-white hover:border-bicimici-green transition-all duration-300">
                    ...
                    <div id="{{ $section->id }}" class="">
                        <ul class="flex flex-col gap-4">
                            ...
                            <li class="group {{ $currentSection && $section->id == $currentSection->id && $currentContent->id == $content->id ? 'active' : '' }}">
                                    ...
                                    <div class="px-4 group-[&.active]:bg-bicimici-black group-[&.active]:border-transparent group-[&.active]:text-white py-[10px] rounded-full border border-bicimici-grey group-hover:bg-bicimici-black transition-all duration-300">
    ...
    <div class="flex-grow overflow-y-auto">
        ...
        <nav class="fixed bottom-0 left-auto right-0 z-30 mx-auto w-[calc(100%-260px)] pt-5 pb-[30px] bg-[#F8FAF9]">
            <div class="px-[30px]">
                <div class="content border border-bicimici-grey rounded-[20px] bg-white p-[12px] flex items-center justify-between">
                    <p class="text-bicimici-text-secondary">Pelajari materi dengan baik, jika bingung maka tanya mentor kelas</p>
                    <div class="buttons flex items-center gap-[12px]">
                        <a href="#" class="rounded-full border border-bicimici-grey px-5 py-[10px] hover:border-bicimici-green transition-all duration-300">
                        ...
                        <a href="{{ route('dashboard.course.learning', [
                                ...
                                ]) }}" class="rounded-full border bg-bicimici-green text-white px-5 py-[10px] hover:drop-shadow-effect transition-all duration-300">
                            <span class="font-semibold">Next Lesson</span>
                        </a>
                        @else
                        <a href="{{ route('dashboard.course.learning.finished', $course->slug) }}" class="rounded-full border bg-bicimici-green text-white px-5 py-[10px] hover:drop-shadow-effect transition-all duration-300">
